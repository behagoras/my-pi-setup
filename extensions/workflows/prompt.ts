...
export const WORKFLOW_TOOL_DESCRIPTION = [
  ...
  '• await agent(prompt, { label?, phase?, schema?, model?, provider?, effort?, harness? }) — run ONE subagent in an isolated context and wait for it. Always resolves to { ok, output, structured?, error? }. Check `ok` before using the result. When you pass a JSON `schema`, `structured` holds the validated object on success. `model`/`provider` override the session model; `effort` sets the thinking level (off|minimal|low|medium|high|xhigh|max). `harness` picks the runtime: "pi" (default, Pi child session with Pi\'s tools) or "claude" (Claude Agent SDK on the Claude subscription, with Claude\'s own tools, where `model` is an alias like "opus" and `provider` does not apply). Children receive normal built-ins and trust-appropriate extensions, settings, skills, and AGENTS.md context, but cannot recursively orchestrate or ask the user.',
